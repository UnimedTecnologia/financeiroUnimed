...
// === SQL com binds ===
$sql = "
SELECT TITULAR.NM_USUARIO TITULAR, TITULAR.CD_CPF CPF_TITULAR, PF.NM_PESSOA USUARIO, PF.CD_CPF CPF, sum(PP.VL_EVENTO) VALOR 
 FROM GP.FATEVECO PP INNER JOIN GP.MOVIPROC MP  ON MP.CD_UNIDADE             = PP.CD_UNIDADE
  ...
  WHERE PP.CD_EVENTO IN ( 70, 71 ) 
    ...
    GROUP BY
    TITULAR.NM_USUARIO, TITULAR.CD_CPF, PF.NM_PESSOA, PF.CD_CPF
";
...
if ($tipo === 'txt') {
    ...
    function formatarLinhaTxt($rowData, &$registrosUnicos) {
        ...
        // Exemplo: "15,00" -> "0000000015,00" (10 posições + vírgula + 2 decimais = 12)
        $partes = explode(',', $valor_formatado);
        $inteiros = str_pad($partes[0], 10, '0', STR_PAD_LEFT);
        ...
        $chaveUnica = $titular . '|' . $cpf_titular . '|' . $usuario . '|' . $cpf . '|' . $valor;
        ...
        if (!isset($registrosUnicos[$chaveUnica])) {
            ...
            return "{$titular};{$cpf_titular};{$usuario};{$cpf};{$valor}\n";
        }
